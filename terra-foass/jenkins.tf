...
//  security_groups             = ["${aws_security_group.vpc-foaas-default.id}"]
  iam_instance_profile        = "foaas-jenkins-ec2-dynamic-slaves"
  vpc_security_group_ids      = ["${aws_security_group.foaas_consul.id}","${aws_security_group.vpc-foaas-default.id}"]
  ...
  tags {
    Name = "tf-foaas-public-jenkins-1"
  }
